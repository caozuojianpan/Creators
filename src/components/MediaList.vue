<template>
   <!-- <section id="refreshContainer" class="mui-content mui-scroll-wrapper">
    ...
    <section >
        ...
        <scroller :on-refresh="refresh" :on-infinite="infinite" ref="myscroller" style="margin-top: 46px">
                ...
                this.$axios({
                    ...
                    if (res.data.code === 1000) {
                        ...
                        if (type === 1) {
                            ...
                            // this.mui('#refreshContainer').pullRefresh().endPulldownToRefresh(false);
                            this.$refs.myscroller.finishInfinite(false);//这个方法是不让它加载了，显示“没有更多数据”，要不然会一直转圈圈

                        } else {
                            this.datas.push(...res.data.data.records);
                            this.$refs.myscroller.finishInfinite(true)
                            // this.mui('#refreshContainer').pullRefresh().endPullupToRefresh(false);
                            // this.mui('#refreshContainer').pullRefresh().enablePullupToRefresh();
                        }
                        // this.mui('#refreshContainer').pullRefresh().refresh(true);
                    } else {
                        this.mui.toast("加载失败")
                    }
